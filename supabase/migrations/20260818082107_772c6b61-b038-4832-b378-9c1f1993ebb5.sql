...
BEGIN
  ...
  LOOP
    IF v_item.purchase_date IS NOT NULL THEN
      v_months_used := DATE_PART('month', AGE(CURRENT_DATE, v_item.purchase_date))
                       + 12 * DATE_PART('year', AGE(CURRENT_DATE, v_item.purchase_date));
      v_annual_depreciation := v_item.unit_cost / v_item.depreciation_years;
      UPDATE public.inventory_items
      SET accumulated_depreciation = LEAST(
            v_annual_depreciation * v_months_used / 12,
            v_item.unit_cost
          )
      WHERE id = v_item.id;
    END IF;
